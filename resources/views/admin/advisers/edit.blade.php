        ...
        <div class="col-md-8 mx-auto">
            ...
            <div class="card shadow-lg mt-4" style="border: none; border-radius: 12px;">
                <div class="card-header" style="background: linear-gradient(135deg, #1B396A 0%, #2D5A96 100%); color: white; border-radius: 12px 12px 0 0; padding: 20px;">
                    <h4 class="mb-0"><i class="bi bi-pencil"></i> Editar Asesor</h4>
                </div>
                <div class="card-body" style="padding: 30px;">
                    <form method="POST" action="{{ route('admin.advisers.update', $adviser->id) }}" novalidate>
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('user_id') is-invalid @enderror" id="user_id" name="user_id" required>
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" @selected($adviser->user_id == $user->id)>{{ $user->nombres }} {{ $user->apa }} - {{ $user->id }}</option>
                                @endforeach
                            </select>
                            @error('user_id')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('especialidad') is-invalid @enderror" id="especialidad" name="especialidad" value="{{ $adviser->especialidad }}">
                            @error('especialidad')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
