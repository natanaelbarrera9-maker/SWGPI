        ...
        <div class="col-md-8 mx-auto">
            ...
            <div class="card shadow-lg mt-4" style="border: none; border-radius: 12px;">
                <div class="card-header" style="background: linear-gradient(135deg, #1B396A 0%, #2D5A96 100%); color: white; border-radius: 12px 12px 0 0; padding: 20px;">
                    <h4 class="mb-0"><i class="bi bi-plus-circle"></i> Nuevo Entregable</h4>
                </div>
                <div class="card-body" style="padding: 30px;">
                    <form method="POST" action="{{ route('admin.deliverables.store') }}" novalidate>
                        @csrf
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('competencia_id') is-invalid @enderror" id="competencia_id" name="competencia_id" required>
                                ...
                                @foreach($competencias as $c)
                                    <option value="{{ $c->id }}" @selected(old('competencia_id') == $c->id)>{{ $c->nombre }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('formatos_aceptados') is-invalid @enderror" id="formatos_aceptados" name="formatos_aceptados" value="{{ old('formatos_aceptados') }}">
